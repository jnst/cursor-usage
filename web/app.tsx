...
import { DEFAULT_METRIC, isMetric } from "../src/core/types.ts";
...
function routeFromHash(defaultTimeZone: string): {
  ...
  return {
    ...
    metric: isMetric(metricParam) ? metricParam : DEFAULT_METRIC,
  };
}

/** Selected Daily Window, Metric, and Analysis Time Zone, kept in sync with the URL hash. */
function useDailyWindowRoute(): {
  ...
  const updateHash = (
    ...
    if (dailyWindow || user || metric !== DEFAULT_METRIC) {
